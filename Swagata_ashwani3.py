...
# Activity 3 - this program includes the steps of Activity 1 plus activity 2 coming up with
# frequency of words for old ticket lists and new ticket list
# and compares frequency
...
def main():
    ...
    # ************** Start of Activity 2 *****************
    # allow the user to decide when ready to go to the next step
    print("Welcome to Project Activity Part 2")
    print()
    inputFlag='n'
    while inputFlag!='y' and inputFlag!='Y':
    ...
    # determining the lenght of new ticket dict
    new_ticket_count=len(new_ticket_clean_freq)

    # looping through new ticket dict and adding to unique list if not part of old ticket dict 
    new_ticket_unique_list=[]
    for n in new_ticket_clean_freq:
        if n not in raw_tickets_clean_freq.keys():
            new_ticket_unique_list.append(n)
    new_ticket_final_count=len(new_ticket_unique_list)

    ratio = new_ticket_final_count / new_ticket_count

    print('Welcome to Activity 3')
    # allow the user to decide when ready to go to the next step
    inputFlag='n'
    while inputFlag!='y' and inputFlag!='Y':
        inputFlag=input("Press 'y' to see final ratio and who the new ticket will be assigned to. 'q' to quit.")
        if inputFlag=='q' or inputFlag=='Q':
            sys.exit()
    print("------------------------------------------------------------------------------------")

    if ratio >= .5:
        print("This ticket contains {:.2%} % new words.  It should be assigned to a Senior Analyst.".format(ratio))
    else:
        print("This ticket contains {:.2%} % new words.  It is safe to assign to a Junior Analyst.".format(ratio))
    print("------------------------------------------------------------------------------------")    
